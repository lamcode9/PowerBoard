...
- [x] **Phase 6 — canvas craft** (2026-07-13). **Root-cause fix:** the selection ring, resize handles, snap guides and marquee live inside the `scale(zoom)` plane, so fixed-px borders blurred/doubled at zoom. `applyCamera` now publishes a live `--zoom` on `.canvas-plane`; every affordance sizes with `calc(px / var(--zoom))` → constant *screen* size at any zoom. **Verified live at 0.8× and 3.05×:** ring = crisp ~1–1.5px accent hairline (glow dropped); four corner handles (nw/ne/sw/se, anchor-aware resize with opposite edge pinned) = steady 7.98px dots, each `aria-label`ed; snap guides + marquee → accent (rose `#f43f5e` gone); connector **midpoint handle** = 8px dot / 16px hit that bends the line via the existing `update_connector` waypoint op (double-click straightens — no new op/schema). Gesture audit passed as-is (⌘0=100%, ⌘1=fit, cursor-centered wheel/pinch zoom, native trackpad momentum). typecheck/build/46 tests green. **Honest gaps:** (a) drag *interactions* (resize-from-corner, midpoint-bend) are code+math-verified but not runtime-verified — the browser harness fires click/mouse, not the pointer events canvas drags need; (b) **pre-existing** connector occlusion flagged not changed — `.connector-layer` z-index 0 < `.artboard-frame` z-index 1, so connectors/handles over an opaque frame are hidden (only visible in frame gaps); raising the layer is UX-sensitive (mockup vs diagram) → surfaced for a decision. Files: `App.tsx`, `styles.css`.
- [ ] Phase 7 per the doc (a11y floor + taste pass — ship gate).
